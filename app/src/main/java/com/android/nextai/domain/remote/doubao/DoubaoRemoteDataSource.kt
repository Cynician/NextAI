...
object DoubaoRemoteDataSource : AIModelDataSource {
    ...
    override suspend fun getAIAnswer(messageList: List<Message>): String {
        return "这是一个用于测试 Markdown 渲染效果的示例文档。它包含了标题、列表、代码块、表格、引用以及各种文本样式。\n" +
                "\n" +
                "## 1. 文本样式测试\n" +
                "\n" +
                "- **粗体文本**：这是加粗的内容。\n" +
    ...
    override suspend fun getAIStreamingAnswer(
        ...
        try {
            ...
            // 模拟逐字或逐词输出
            val words = fullText.chunked(2) // 每次发送 2 个字符，模拟 Token

            for (word in words) {
                delay(Random.nextLong(50, 150)) // 模拟网络波动或计算时间
